...
class DownloadYoutubeThread(QThread):
    ...
    def download_yt_video(self, stream):
        ...
        try:
            ...
            msg = 'Started loading file {0} with size: {1:.2f} MB.....'.format(video_title, file_size_MB)
            self.start_video_loading.emit(msg)
            stream.download(self.save_directory)
            if self.convert_mp3:
                command = r'.\downloads\youtube\ffmpeg-4.0.1-win32-static\bin\ffmpeg -i "{0}/{1}" -f mp3 "{0}/{2}.mp3"'
                command = command.format(self.save_directory, stream.default_filename,video_title)
                print(command)
                subprocess.call(command)
                self.converted_to_mp3.emit('Video additionally converted to mp3: ' + video_title + '.mp3')
            msg = 'Successfully downloaded the video "{0}" which is located at "{1}"\n\n'.format(video_title, self.save_directory)
            return True, msg
        except:
            exc_type, exc_obj, exc_tb = sys.exc_info()           
            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
